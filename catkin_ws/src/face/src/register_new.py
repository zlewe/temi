...
file_path = os.path.join(src_path, 'players')

# Re-direct path to player folder
os.chdir(file_path)

# player list 
new_resgister = {'name':None, 'faces':[]}
...
# get skeleton
def frame_cb(msg):
    global target_image, new_resgister
    ...
    for skeleton in msg.persons:
        ...
        for (x,y,w,h) in faces:
            ...
            face_count = len(new_resgister['faces'])
            print('The %dth picture was taken...'%(face_count))
            if face_count>100:
                stopDetect()
                return
...
def name_cb(msg):
    global game_pub, new_resgister

    players = getAllRegisterPlayer()
    ...
    if player_name in players:
        ...
        new_resgister = {'name':player_name, 'faces':[]}
